...
public class Computer_AI {
	...
	public static boolean AI_Block() {
		//i tryed to add 
		//if()//Connect 4 from right to left horizontal add coin to right if space available
		//Connect 4 from left to right horizontal add coin to left if space available
		//Connect 4 from down to up  add coin on top of the section
		//Connect 4 from right to left diagonal upwards add coin to right if space available and base made(one down from the position where the connect 5 will happen is not empty )
		//Connect 4 from left to right diagonal downwards add coin to left if space available and base made(one down from the position where the connect 5 will happen is not empty )
...
//						else
						{
							...
							{
								if(LogicalDecisionclass.getCoinStatus(i+1,j)!="0")
				...
				if(i < Connect5.board_lenght_col - 4 && j < Connect5.board_width_row - 4)
					{
						...
						{   
							...
							if(LogicalDecisionclass.getCoinStatus(i,j)=="0" &&j+4<Connect5.board_width_row&&i+4<Connect5.board_lenght_col)
							{
								if(LogicalDecisionclass.getCoinStatus(i+1,j)!="0")
								{getAItoaddcoin(i,j);;System.out.println("blocked diag on left 4");return true;}
							}
						}
					}
